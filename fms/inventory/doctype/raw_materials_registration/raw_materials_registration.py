...
class RawMaterialsRegistration(Document):
	...
	def validate_name_not_item_name(self):
		...
		if frappe.db.exists("Item", {"item_name": self.name1}):  # Corrected field name
			...
			frappe.throw(
				f"Item name '{self.name1}' already exists as an Item name. Please choose a different name.",
	...
	def on_submit(self):
		...
		try:
			...
			stock_ledger.remarks = f"Initial stock for Raw Matrials: {self.name}"  # Add a descriptive remark
			...
			frappe.log_error(f"Error creating Stock Ledger: {e}", "Raw Matrial")  # Log the error for debugging
